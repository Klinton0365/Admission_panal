...
class User
{
  ...
  private function _get_data($var)
  {
    ...
    //$sql = "SELECT `$var` FROM `Studbio` WHERE `id` = $this->id";
    $sql = "SELECT `$var` FROM `Student` WHERE `id` = $this->id";
  ...
  private function _set_data($var, $data)
  {
    ...
    //$sql = "UPDATE `Studbio` SET `$var`='$data' WHERE `id` = $this->id;";
    $sql = "SELECT `$var` FROM `Student` WHERE `id` = $this->id";
  ...
  public static function signup($user, $phone, $email, $pass)
  {
    ...
    $sql = "INSERT INTO `Student` (`username`, `password`, `email`, `phone`, `blocked`, `active`)
  ...
  public static function login($user, $pass)
  {
    ...
    $stmt->execute();
    // $query = "SELECT * FROM `Student` WHERE `username` = '$user'";  //Genaral query statements
  ...
  public static function register($user, $email, $phone, $date){
    $conn = Database::getConnection();
    $sql = "INSERT INTO `Studbio` (`username`, `email`, `phone`, `date`, `course`,`gender`)
    VALUES ('$user','$email','$phone','$date','course','gender');";
  ...
  public function __construct($username)
  {
    ...
    $this->id = null;
    $sql = "SELECT `id` FROM `Student` WHERE `username`='$username' OR `id`='$username' LIMIT 1"; //OR `id`='$username'
    $result = $this->conn->query($sql);
    if ($result->num_rows) {
      $row = $result->fetch_assoc();
